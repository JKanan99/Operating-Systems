...
//  Customer() = default;
  ...
  const vector_<int>& get_alloc() const { 
    return alloc; 
  }

  const vector_<int>& get_max() const { 
    return max_; 
  }

  const vector_<int>& get_need() const { 
    return need; 
  }

  void alloc_req(  const vector_<int>& req) { 
    alloc += req;  need -= req; 
  }

  void dealloc_req(const vector_<int>& req) { 
    alloc -= req;  need += req; 
  }
  ...
  vector_<int> create_req() { 
    ...
    for (size_t i = 0; i < alloc.size(); ++i) {
      ...
      int val = (int)((rand() % need[i])+1); //only request what they still need(not 3) (have less denials)
      //original code: int val = (int)((rand() % 3) + 1); 
      req.push_back(val);
    }
    return req;
  }
